...
def validate_rpo_args(args):
    if not args.use_arr:
        raise ValueError("RPO training only supports ARR reward. Please pass --use_arr.")
...
def apply_lora(transformer, args):
    if get_peft_model is None or LoraConfig is None:
        raise ImportError("LoRA training requires `peft`. Please install peft.")
...
def sample_reference_model(
    ...
    for index, batch_idx in enumerate(batch_indices):
        ...
        image_path = f"./images/flux_{rank}_{index}.png"
        if index % 2 == 0:
            pair_item = {
...
def train_one_step(
    ...
    if args.use_group and args.use_arr:
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=int,
        default=None,   
        help="latent temporal length; keep 1 for image training",
    )
    parser.add_argument(
        "--sampling_steps",
        type=int,
