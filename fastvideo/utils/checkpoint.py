...
def save_hf_lora_checkpoint(transformer, optimizer, rank, output_dir, step, epoch):
    """Save a PEFT LoRA adapter in the standard Hugging Face layout.
    ...
    The checkpoint directory contains:
      ...
      - trainer_state.json
    """
    main_print(f"--> saving HF LoRA checkpoint at step {step}")
    with FSDP.state_dict_type(
            transformer,
            StateDictType.FULL_STATE_DICT,
            FullStateDictConfig(offload_to_cpu=True, rank0_only=True),
    ...
    if rank <= 0:
        save_dir = os.path.join(output_dir, f"checkpoint-{step}-{epoch}")
        os.makedirs(save_dir, exist_ok=True)
        unwrapped = _unwrap_fsdp(transformer)
        ...
        torch.save(lora_optim_state, os.path.join(save_dir, "lora_optimizer.pt"))
        with open(os.path.join(save_dir, "trainer_state.json"), "w") as f:
            json.dump({"step": step, "epoch": epoch}, f, indent=4)
    main_print(f"--> HF LoRA checkpoint saved at step {step}")
