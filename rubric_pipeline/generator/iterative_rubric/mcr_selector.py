...
class SuperFastAdaptiveMCR2:
    ...
    def __init__(self, batch_size: int = 20, embed_dim: int = 2560):
    ...
    def generate_embeddings_batch(self, texts: List[str]) -> np.ndarray:
        ...
        for i in tqdm(
            ...
            try:
                rsp = self.model.encode(batch_texts, prompt_name="query")
                embeddings = [list(rsp_i) for rsp_i in rsp]
                all_embeddings.extend(embeddings)
            except Exception as e:
                logger.error(f"Error generating embeddings: {e}")
                all_embeddings.extend([np.zeros(self.embed_dim) for _ in batch_texts])
