...
def get_all_grader_info() -> List[Dict[str, Any]]:
    """Collect information for graders defined under the rubric_pipeline/graders folder."""
    t0 = time.time_ns()
...
def _get_defs_of_classes_having_parent(
    ...
    # /path/to/project/rubric_pipeline/graders/foo/my_grader.py ->
    # rubric_pipeline/graders/foo/my_grader.py ->
    # rubric_pipeline/graders/foo/my_grader ->
    # rubric_pipeline.graders.foo.my_grader
    module_path = str(py_file.relative_to(project_root_folder).with_suffix("")).replace(os.sep, ".")
