...
from rubric_pipeline.models.schema.oai.response import ChatResponse
from rubric_pipeline.utils.utils import repair_and_load_json
    ...
    async def validate(self) -> None:
        ...
        try:
            ...
            if "not found" in str(e).lower() or "does not exist" in str(e).lower():
                raise ValueError(f"❌ 模型 '{self.model}' 不存在或你没有权限访问！请检查模型名是否正确（gpt-5 / gpt-5.4 等）") from e
            else:
                raise RuntimeError(f"❌ 模型验证失败: {e}") from e
    ...
    async def achat(
        ...
        **kwargs: Any,
        ...
        if structured_model:
            ...
            # Use simple json_object format for OpenAI-compatible endpoints that
            # do not implement the SDK's Pydantic parse helper.
            if "qwen" in self.model.lower() or "gemini" in self.model.lower():
                logger.warning(
                    "{} uses JSON-object structured output fallback.",
                    self.model,
                )
            ...
            #     structured_model['properties'] = {}
            #     structured_model['properties']['metadata'] =  {"type": "object", "additionalProperties": False}

            kwargs["response_format"] = structured_model

            if not self.stream and isinstance(structured_model, dict):
