...
from rubric_pipeline.graders.base_grader import (
    BaseGrader,
    GraderMode,
    GraderRank,
    GraderScore,
)
from rubric_pipeline.graders.schema import GraderRankCallback, GraderScoreCallback
    ...
    async def aevaluate(
        ...
        **kwargs: Any,
        ...
        Example:
            ...
            >>> from rubric_pipeline.model.dashscope_llm import DashScopeLLM
            >>> grader = LLMGrader(
            ...     name="helpfulness",
        ...
        task_type = normalize_task_type(params.get("task_type", TASK_T2I))

        if image_paths is None:
            image_paths = extract_image_paths(params, task_type)
            image_has_base = has_base_image(params, task_type)
        elif isinstance(image_paths, str):
            image_paths = [image_paths]
            image_has_base = bool(params.get("has_base_image", False))
        else:
            image_has_base = bool(params.get("has_base_image", False))

        if "has_base_image" not in params and task_type == TASK_IMAGE_EDIT:
            if self.mode == GraderMode.POINTWISE and len(image_paths) == 2:
                image_has_base = True
            elif self.mode == GraderMode.LISTWISE and len(image_paths) >= 3:
                image_has_base = True

        num_outputs = count_outputs(list(image_paths), task_type, has_base=image_has_base)
        ranking_mode = "Pairwise" if num_outputs == 2 else "Listwise"
        params.setdefault("num_outputs", num_outputs)
        params.setdefault("ranking_mode", ranking_mode)
        if "sample_content" not in params:
            query = params.get("query") or params.get("prompt") or params.get("instruction") or ""
            params["sample_content"] = f"Instruction: {query}" if task_type == "image_edit" else f"Caption: {query}"

        messages = add_images_to_messages(
            messages=self.template.format(language=self.language, **params),
            image_paths=list(image_paths),
            task_type=task_type,
            has_base=image_has_base,
        )
        ...
        if self.mode == GraderMode.LISTWISE:
            ...
            result = GraderRank(
                name=self.name,
                rank=[int(value) for value in rank],  # type: ignore
